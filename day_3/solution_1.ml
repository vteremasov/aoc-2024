let input_file = "./day_3/input/input1.txt"
...
let parse_mul_str str =
  if String.length str >= 4 && String.sub str 0 4 = "mul(" then
...
let rec parse_muls str =
  match str with
  | "" -> []
  | _ -> parse_mul str :: parse_muls (String.sub str 1 (String.length str - 1))

let parsed_muls = List.filter_map (fun x -> x) (parse_muls (read_lines input_file));;
