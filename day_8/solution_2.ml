...
let permutates lst = List.map (fun (c, l) -> (c, permutate l)) lst;;
...
let all map c fn v d =
  let res = ref [] in
  let p = ref v in
  while is_untinode map c (fn !p d) do
    res := List.append !res [(true, fn !p d)];
    p := fn !p d;
  done;
  !res
;;
...
let get_count f =
  ...
  |> (fun map -> collect_pos map |> group_by |> permutates |> fun coords -> (map, coords))
  |> fun (map, coords) -> 
      List.fold_left (fun acc (c, lst) -> 
        acc @ List.map (fun (a, b, d) -> 
          (all map c vec_add a d) @
          (all map c vec_sub b d) @
          [(true, a);(true, b)]
        ) lst
      ) [] coords
  |> List.flatten
